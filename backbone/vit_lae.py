...
def vit_b16_in21k_lae(pretrained=True, **kwargs):
    # (1) Import or reference default_cfgs from this file
    from .vit_lae import default_cfgs  # or wherever your local dict lives

    model_kwargs = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs
    )
    # (2) Make a local variable to hold that config
    custom_cfg = default_cfgs["vit_b16_in21k"]

    # Now pass pretrained_cfg=custom_cfg to override timm's default logic
    model = _create_vision_transformer(
        "vit_b16_in21k",
        pretrained=pretrained,
        pretrained_cfg=custom_cfg,  # <--- the critical line
        **model_kwargs
    )
    return model
