...
from backbone.vit_newlae import _create_vision_transformer
...
'''
@register_model
def vit_b16_in21k(pretrained=False, **kwargs):
    """Register 'vit_b16_in21k' to load from the default_cfgs dict entry."""
    model = _create_vision_transformer(
        "vit_b16_in21k",
        pretrained=pretrained,
        default_cfg=default_cfgs["vit_b16_in21k"],  # This points to that .npz URL
        **kwargs
    )
    return model
'''
def vit_b16_in21k_adapter(pretrained=True, **kwargs):
    """
    1) Build the adapter-based VisionTransformer (which has q_proj/k_proj/v_proj).
    2) If pretrained=True, call _create_vision_transformer("vit_b16_in21k", pretrained=True)
       to get a single-qkv model, then rename 'qkv.*' to 'q_proj.*','k_proj.*','v_proj.*'
       in its state_dict, and load into the adapter-based model.
    3) Freeze everything except the adapter if desired.
    """
    # 1) Build your adapter-based model (exactly as you do in the other _ease functions)
    model = VisionTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4,
        qkv_bias=True,  # we do actually split into q_proj, k_proj, v_proj below
        norm_layer=partial(nn.LayerNorm, eps=1e-6),
        **kwargs
    )

    if pretrained:
        # 2a) Build a plain model with single qkv param using your 'vit_newlae._create_vision_transformer'
        temp_model = _create_vision_transformer(
            "vit_b16_in21k",
            pretrained=True,
            pretrained_cfg=default_cfgs["vit_b16_in21k"],
            num_classes=0,
        )

        # 2b) Extract its state dict
        state_dict = temp_model.state_dict()

        # 2c) Rename the qkv.* keys to q_proj, k_proj, v_proj
        #     Similar to the logic in vit_base_patch16_224_in21k_ease
        for key in list(state_dict.keys()):
            if 'qkv.weight' in key:
                qkv_weight = state_dict.pop(key)
                q_weight = qkv_weight[:768]
            ...
            # If your MLP layers also differ in naming, do the 'mlp.fc->fc' rename:
            if 'mlp.fc' in key and key in state_dict:
                fc_weight = state_dict.pop(key)
                new_key = key.replace('mlp.', '')
                state_dict[new_key] = fc_weight

        # 2d) Load into your adapter-based model
        msg = model.load_state_dict(state_dict, strict=False)
        print("Loaded vit_b16_in21k weights (qkv-split) into adapter model:", msg)

        # 2e) Freeze everything but adapter
        for name, p in model.named_parameters():
            if name in msg.missing_keys:
                p.requires_grad = True
            else:
