...
@register_model
def vit_base_patch16_224_in21k_newlae(pretrained=False, **kwargs):
...
@register_model
def vit_base_patch16_224_newlae(pretrained=False, **kwargs):
...
@register_model
def vit_b16_in21k_newlae(pretrained=False, **kwargs):
    model_kwargs = dict(
        patch_size=16,
        embed_dim=768,
        depth=12,
    ...
    # If user says pretrained=True, we inject the config from our local dict:
    if pretrained:
        kwargs.setdefault("pretrained_cfg", default_cfgs["vit_b16_in21k"])
    ...
    # If you want to return only the CLS token in forward_features:
    # (Assuming timm’s VisionTransformer is used under the hood)
    old_forward_features = model.forward_features

    def new_forward_features(x):
        out = old_forward_features(x)  # [bs, tokens, dim] = [bs, 197, 768]
        cls_only = out[:, 0]  # [bs, 768]
        return cls_only
