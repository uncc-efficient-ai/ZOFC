...
class InfLoRA_domain(BaseLearner):
    ...
    def _train(self, train_loader, test_loader):
        ...
        # inflora_domain.py, inside _train after (optional) DataParallel wrapping
        net = self._unwrap(self._network)  # <-- works for both DP and non-DP
        base_params = net.image_encoder.parameters()
        base_fc_params = [p for p in net.classifier_pool.parameters() if p.requires_grad]
        ...
        if self.optim == 'sgd':
            ...
            optimizer = optim.Adam(network_params,  # <-- use param groups
