...
class BaseLearner(object):
    def __init__(self, args):
        ...
        # ---- robust device parsing ----
        raw = args.get("device", None)

        def to_cuda_index(x):
            # returns an int cuda index, or None if not cuda
            if isinstance(x, torch.device):
                if x.type == "cuda":
                    return 0 if x.index is None else int(x.index)
                else:
                    return None
            if isinstance(x, int):
                return x
        ...
        # map to cuda indices, filter invalid / out-of-range
        cuda_ids = []
        for d in dev_list:
            idx = to_cuda_index(d)
            if idx is not None:
        ...
        cuda_ids = [i for i in cuda_ids if i < available]

        if torch.cuda.is_available() and len(cuda_ids) >= 1:
            self._device = torch.device(f"cuda:{cuda_ids[0]}")
            # Only enable DataParallel if 2+ valid ids are given
            self._multiple_gpus = cuda_ids[1:] and cuda_ids or []
            if len(self._multiple_gpus) <= 1:
                self._multiple_gpus = []
        else:
            self._device = torch.device("cpu")
            self._multiple_gpus = []
        # ---- end robust device parsing ----
