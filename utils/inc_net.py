...
def get_backbone(args, pretrained=False):
    ...
    if name == "pretrained_vit_b16_224" or name == "vit_base_patch16_224":
        ...
        model.out_dim = 768
        return model.eval()
    elif name == "pretrained_vit_b16_in21k":
        from backbone import vit_newlae
        # This is the 'plain' single-qkv model.
        #   e.g. `vit_b16_in21k_newlae(...)` loads the official checkpoint:
        model = vit_newlae.vit_b16_in21k_newlae(pretrained=True, num_classes=0)
        # or do `_create_vision_transformer("vit_b16_in21k", pretrained=True, num_classes=0)`
        ...
        if args["model_name"] == "aper_adapter" or args["model_name"] == "ranpac" or args["model_name"] == "fecam":
            ...
            if name == "pretrained_vit_b16_224_adapter":
                ...
                model = vit_adapter.vit_base_patch16_224_in21k_adapter(num_classes=0,
                    global_pool=False, drop_path_rate=0.0, tuning_config=tuning_config)
                model.out_dim=768
            elif name == "pretrained_vit_b16_in21k_adapter":
                model = vit_adapter.vit_b16_in21k_adapter(num_classes=0,
    ...
    # L2P
        ...
        if args["model_name"] == "l2p":
            ...
            if name == "vit_b16_in21k_l2p":
                # Call your local function directly:
                model = vit_l2p.vit_b16_in21k_l2p(
    ...
    # Coda_Prompt
        ...
        if args["model_name"] == "ease" :
            ...
            if name == "vit_base_patch16_224_ease":
                ...
                model = vit_ease.vit_base_patch16_224_in21k_ease(num_classes=0,
                    global_pool=False, drop_path_rate=0.0, tuning_config=tuning_config)
                model.out_dim=768
            elif name == "vit_b16_in21k_ease":

                model = vit_ease.vit_b16_in21k_ease(num_classes=0,
                    global_pool=False, drop_path_rate=0.0, tuning_config=tuning_config)
                model.out_dim=768
            else:
                raise NotImplementedError("Unknown type {}".format(name))
        ...
        if name == "vit_base_patch16_224_lae":
            model = vit_lae.vit_base_patch16_224_lae(pretrained=True)
        elif name == "vit_base_patch16_224_in21k_lae":
            model = vit_lae.vit_base_patch16_224_in21k_lae(pretrained=True)
        elif name == "vit_b16_in21k_lae":
            model = vit_lae.vit_b16_in21k_lae(pretrained=True)
        else:
            raise ValueError(f"Unknown lae model: {name}")
        return model


    elif '_newlae' in name:
        from backbone import vit_newlae
        if name == "vit_base_patch16_224_newlae":
            model = vit_newlae.vit_base_patch16_224_newlae(pretrained=True)
        elif name == "vit_base_patch16_224_in21k_newlae":
            model = vit_newlae.vit_base_patch16_224_in21k_newlae(pretrained=True)
        elif name == "vit_b16_in21k_newlae":
            model = vit_newlae.vit_b16_in21k_newlae(pretrained=True)
        else:
            raise ValueError(f"Unknown newlae model: {name}")
        return model
...
class PromptVitNet(nn.Module):
    ...
    def get_original_backbone(self, args):
        # Check if backbone_type is the local vit_b16_in21k_l2p
        if args["backbone_type"] == "vit_b16_in21k_l2p":
            from backbone import vit_l2p  # import your local vit_l2p
            ...
            # Whether you call .eval() here depends on your usage
            return model.eval()
        else:
            # Otherwise, fall back to timm
...
class LAE(nn.Module):
    ...
    def create_pets(self):
        ...
        if self.pet_cls == "Adapter":
            ...
            return nn.ModuleList([Adapter(embed_dim=embed_dim, down_sample=self.down_sample_dim, mode="parallel") for _ in range(n)])
    ...
    def attach_pets_vit(self, pets: nn.ModuleList):
        ...
        if self.pet_cls == "Adapter":
            for i, b in enumerate(self.adapt_blocks):
                self.backbone.blocks[b].attach_adapter(mlp=pets[i])
            return
        if self.pet_cls == "LoRA":
            for i, b in enumerate(self.adapt_blocks):
...
class NewLae(nn.Module):
    ...
    def create_pets(self):
        ...
        embed_dim = self.backbone.embed_dim
        if self.pet_cls == "Prefix":
            from backbone.vit_newlae import Prefix
            return nn.ModuleList([Prefix(length=self.pet_length, dim=embed_dim, key_scale=1.0, val_scale=1.0) for _ in range(n)])
        if self.pet_cls == "Adapter":
            from backbone.vit_newlae import Adapter
            #return nn.ModuleList( [Adapter(embed_dim=embed_dim, down_sample=self.down_sample_dim, mode="parallel") for _ in range(n)])

            return nn.ModuleList([Adapter(embed_dim=embed_dim, down_sample=self.down_sample_dim, scale=0.1, mode="parallel") for _ in range(n)])
        if self.pet_cls == "LoRA":
            from backbone.vit_newlae import KVLoRA
            return nn.ModuleList([KVLoRA(in_features=embed_dim, out_features=embed_dim, scale=0.1) for _ in range(n)])
